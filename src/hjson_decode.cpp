...
static std::string _errAt(Parser* p, std::string message) {
  ...
  return message + " at line " + std::to_string(line) + "," + std::to_string(col) + " >>> " +
         std::string((char*)p->data + p->at - col, samEnd);
}
...
static void _toUtf8(std::vector<char>& res, uint32_t uIn) {
  if (uIn < 0x80) {
    res.push_back(static_cast<char>(uIn));
  } else if (uIn < 0x800) {
...
static std::string _readKeyname(Parser* p) {
  ...
  for (;;) {
    if (p->ch == ':') {
      ...
      } else if (space >= 0 && size_t(space) != name.size()) {
        p->at = start + space;
      ...
      if (_isPunctuatorChar(p->ch)) {
        throw syntax_error(
            _errAt(p, std::string("Found '") + (char)p->ch +
                          std::string("' where a key name was expected (check your syntax or use quotes if "
                                      "the key name includes {}[],: or whitespace)")));
      }
      name.push_back(p->ch);
    }
    _next(p);
...
static Value _readTfnns(Parser* p) {
  if (_isPunctuatorChar(p->ch)) {
    throw syntax_error(_errAt(p, std::string("Found a punctuator character '") + (char)p->ch +
                                     std::string("' when expecting a quoteless string (check your syntax)")));
  }
  auto chf = p->ch;
...
Value Unmarshal(const char* data, size_t dataSize) {
  Parser parser = {(const unsigned char*)data, dataSize, 0, ' '};
...
Value Unmarshal(const std::string& data) { return Unmarshal(data.c_str(), data.size()); }
