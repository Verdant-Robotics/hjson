...
Value::ValueImpl::ValueImpl(std::string&& input)
    : type(IMPL_STRING)
    , p(new std::string(std::move(input))) {}
...
Value::ValueImpl::ValueImpl(Type _type) {
  switch (_type) {
    ...
    case MAP:
      ...
      break;
    default:
      assert(!static_cast<bool>("Unknown type"));
      break;
  }
}
...
Value::Value(std::string&& input)
    : prv(std::make_shared<ValueImpl>(std::move(input))) {}
...
const Value Value::operator[](const char* input) const { return operator[](std::string(input)); }

MapProxy Value::operator[](const char* input) { return operator[](std::string(input)); }
...
bool Value::operator==(bool input) const { return operator bool() == input; }

bool Value::operator!=(bool input) const { return !(*this == input); }

bool Value::operator==(double input) const { return operator double() == input; }

bool Value::operator!=(double input) const { return !(*this == input); }

bool Value::operator==(int input) const {
  return (prv->type == ValueImpl::IMPL_INT64 ? prv->i == input : operator double() == input);
}

bool Value::operator!=(int input) const { return !(*this == input); }

bool Value::operator==(const char* input) const { return !strcmp(operator const char*(), input); }

bool Value::operator!=(const char* input) const { return !(*this == input); }

bool Value::operator==(const std::string& input) const { return operator const std::string() == input; }

bool Value::operator!=(const std::string& input) const { return !(*this == input); }
...
bool Value::operator>(const char* input) const { return operator const std::string() > input; }
...
double Value::operator+(int input) const { return operator double() + input; }

double Value::operator+(double input) const { return operator double() + input; }

std::string Value::operator+(const char* input) const { return operator const std::string() + input; }

std::string Value::operator+(const std::string& input) const { return operator const std::string() + input; }
...
double Value::operator-(const Value& other) const { return operator double() - other.operator double(); }
...
bool Value::defined() const { return prv->type != ValueImpl::IMPL_UNDEFINED; }

bool Value::empty() const {
  return (prv->type == ValueImpl::IMPL_UNDEFINED || prv->type == ValueImpl::IMPL_HJSON_NULL ||
...
bool Value::is_int64() const { return prv->type == ValueImpl::IMPL_INT64; }
...
void Value::push_back(Value&& other) {
  ...
  ((ValueVec*)prv->p)->push_back(std::move(other));
}
...
void Value::insert(const std::string& key, Value&& val) {
  switch (prv->type) {
    case ValueImpl::IMPL_UNDEFINED:
      prv->~ValueImpl();
      // Recreate the private object using the same memory block.
      ...
      [[fallthrough]];
    case ValueImpl::IMPL_MAP:
      ((ValueVecMap*)prv->p)->m.insert_or_assign(key, std::move(val));
      break;
    default:
      throw type_mismatch("Must be of type MAP for that operation.");
  }
...
void Value::insert(std::string&& key, Value&& val) {
  switch (prv->type) {
    case ValueImpl::IMPL_UNDEFINED:
      ...
      [[fallthrough]];
    case ValueImpl::IMPL_MAP:
      ((ValueVecMap*)prv->p)->m.insert_or_assign(std::move(key), std::move(val));
      break;
    default:
      throw type_mismatch("Must be of type MAP for that operation.");
  }
...
size_t Value::erase(const std::string& key) {
  ...
  if (ret > 0) {
    ...
    if (it == v->end()) {
      assert(!static_cast<bool>("Value found in map but not in vector"));
    } else {
      v->erase(it);
    }
  }
...
size_t Value::erase(const char* key) { return erase(std::string(key)); }
...
MapProxy::MapProxy(std::shared_ptr<ValueImpl> _parent, std::shared_ptr<ValueImpl> _child,
